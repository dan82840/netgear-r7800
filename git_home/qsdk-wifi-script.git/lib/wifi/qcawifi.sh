...
reload_time_qcawifi() {
	local devices="$2"
	local wifi_reload_time=0
	local wifi_reload_vap_time=7
	local module_reload_time=16
	local module_reload=$(uci_get wireless qcawifi module_reload 1)

	test "$module_reload" = "1" && wifi_reload_time=$module_reload_time
...
disable_qcawifi() {
	...
	band_type=`grep "^[ga]_device" /etc/ath/wifi.conf | grep $phy /etc/ath/wifi.conf | cut -d_ -f1`
	if [ "$band_type" = "g" ]; then
		rm /tmp/WLAN_uptime
