...
as_well=" as well?"
yes_readyshare_devname="If you choose yes, your readyshare storage access path would be updated to be your device name."
no_readyshare_devname="If you choose no, readyshare storage access path will not be updated."
...
h_UPNP_media_tivo="<BODY bgcolor=#0099cc><P><font size=4><B>Media Server (Settings) Help</B></font></P> <P>Setting this device as a ReadyDLNA Media server enables playback of Videos Movies and Pictures on DLNA/UPnP AV compliant Media Players like Xbox360, Playstation and NETGEAR's Digital Entertainer Live.</p><p>ReadyDLNA implies this device serves media in DLNA compatible form to DLNA/UPnP AV compliant Media Players </p> <ul><li><a name=turnon></a><b>Enable Media Server</b> enables this device to act as a Media server.</li><li><a name=tivo></a><b>Enable TiVo® support</b> Please visit http://www.readynas.com/?p=1428 to see how to use this feature with your TiVo device.</li><li><a name=name></a><b>Media Server name</b> is the name that shows up on Media Players</li></ul> <a name=scan></a><p><b>Content Scan</b></p><ul><li><b>Automatically update database </b> Scans for Media files whenever new files are added to the ReadySHARE USB Storage. Only shared folder with “All – no password” in Read Access can be scanned for Media files.</li><li>You can also click <b>Rescan media files</b> to delete current database and do a whole new scan immediately.</li></ul></p></BODY>"
...
display_top_5_band="Display only top 5 application bandwidth"
...
vpn_mac_step2="Download the client utility from <a class='linktype' href='http://openvpn.net/index.php/download/community-downloads.html' target='_blank'>http://openvpn.net/index.php/download/community-downloads.html</a> and install it on the devices where you want to run the VPN client."
vpn_mac_step4="Unzip the configuration files you have just downloaded and copy them to the folder where the VPN client is installed on your device."
...
vpn_iphone_step3="Click the below button to download the configuration files for your VPN clients."
...
led_en_blinking="Enable blinking on Internet LED, LAN LED, Wireless LED, USB LED and eSATA LED when data traffic is detected"
led_dis_blinking="Disable blinking on Internet LED, LAN LED, Wireless LED, USB LED and eSATA LED when data traffic is detected"
...
h_performance_database="<a name=performance></a><p><b>Performance Optimization Database</b></p><p>The performance optimization database allows the router to detect the types of devices that are connected to the router and the applications the devices are running in order for the router to automatically set the Dynamic QoS priority. </p><a name=auto_update></a><p><b>Automatically update performance optimization database</b></p><p>You can set the router to automatically update its performance optimization database to the latest version if QoS is enabled.</p><p><b>Note:</b> You can click the <b>Update Now</b> button even if you set the router to automatically update its performance optimization database.  </p><p>The router displays the following performance optimization database information:</p><ul><li><b>Version.</b> The current version of the database on the router.</li><li><b>Release Date.</b> The date that the current database on the router was released.</li></ul>"
...
adv_qos="<b>Manual QoS:</b> manual set up of QoS rules"
...
rae_learn_router_ap="<p>In router mode, the only device that can be connected to your existing gateway is your new NETGEAR router. You must connect all of your devices to the new NETGEAR router. If your existing gateway has a WiFi function, disable it.</p><p><b>AccessPoint</b></p>"
